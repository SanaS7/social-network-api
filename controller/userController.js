...
const userController = {
    ...
    deleteUser(req, res) {
        ...
        })).then(() => res.json({ message: 'User and associated apps deleted successfully!' })).catch((err) => res.status(500).json(err));
    },
    // getUserById,
    getUserById(req, res) {
        User.findOne({ _id: req.params.id }).then((user) => !user ? res.status(404).json({ message: 'No user with this ID' }) : res.json(user)).catch((err) => res.status(500).json(err));
    },
    ...
    addFriend(req, res) {
        ...
        User.findOneAndUpdate({
            ...
            runValidators: true,
            new: true
        }).then((user) => !user ? res.status(404).json({ message: 'No friend found with ID :(' }) : res.json(user)).catch((err) => res.status(500).json(err));
    },
